...
function SearchBar(props){
    ...
    console.log(jobCriteria);
     ...
     const search = async() => {
        await props.fetchJobChaser(jobCriteria);
    }
    ...
    return (
        <div className=" flex gap-4 my-10 justify-center px-10">
            <select onChange={handleChange} name="title" value={jobCriteria.title} className="w-64 py-3 pl-4 bg-zinc-200 font-semibold rounded-md">
                ...
                <option value="iOS Devloper">iOS Developer</option>
                <option value="Frontend Developer">Frontend Developer</option>
                <option value="Backend Devloper">Backend Developer</option>
                <option value="Android Devloper">Android Developer</option>
                <option value=" Devloper Advactor"> Developer Advactor</option>
            </select>
            ...
            <button onClick={search} className=" w-64 bg-blue-500 text-white font-bold py-3 rounded-md">Serach</button>
        </div>
    )
}
